...
public class CommandMulta implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("preset") && args.length >= 2){
            // /multa preset <nombre> <jugador>
            String presetName = args[1].toLowerCase();
            Map<String, Object> presets = plugin.getConfig().getConfigurationSection("multas.presets").getValues(false);
            if (!presets.containsKey(presetName)){ p.sendMessage("Preset no encontrado"); return true; }
            if (args.length < 3){ p.sendMessage("Uso: /multa preset <nombre> <jugador>"); return true; }
            Player target = Bukkit.getPlayer(args[2]);
            if (target == null){ p.sendMessage("Jugador no encontrado"); return true; }
            Object node = presets.get(presetName);
            try {
                // node is a map-like object; read price and reason
                String path = "multas.presets." + presetName + ".precio";
            } catch (Exception ex) { ex.printStackTrace(); p.sendMessage("Error al leer preset"); }
            return true;
        }
        ...
        try{
            ...
            p.sendMessage("§aMulta creada (id: " + id + ") para §e" + target.getName());
            target.sendMessage("§cHas recibido una multa de $" + amount + " por: §7" + reason);
